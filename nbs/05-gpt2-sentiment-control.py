...
output = sentiment_model.forward(sentiment_tokenizer.encode(text, return_tensors="pt").to(device))
print(output)
output = output[0][:, 1].detach()
print(output)
reward = pos_logit_to_reward([output], [ctrl_str[0]])
print(reward)
reward = pos_logit_to_reward([output], [ctrl_str[2]])
print(reward)

gpt2_tokenizer = GPT2Tokenizer.from_pretrained(config['tk_name'])

df['tokens'] = df['review'].progress_apply(
    lambda x: gpt2_tokenizer.encode(' ' + x, return_tensors="pt").to(device)[0, :config['txt_in_len']])
...
def test():
    gpt2_model = GPT2HeadWithValueModel.from_pretrained(save_dir)
    _ = gpt2_model.to(device)
